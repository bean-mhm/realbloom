...
int main(int argc, char* argv[])
{
    ...
    if (!parseFailed)
    {
        ...
        for (uint32_t i = 0; i < cgInput.numChunks; i++)
        {
            ...
            if (cgFinalSuccess)
            {
                logAdd(LogLevel::Info, stringFormat(
                    "Chunk %d/%d (%d points) was successful. Renderer: \"%s\"", i + 1, cgInput.numChunks, cgData.numPoints, cgData.gpuName.c_str()));
            } else
            {
                logAdd(LogLevel::Error, stringFormat(
                    "Chunk %d/%d (%d points) was failed. Renderer: \"%s\"", i + 1, cgInput.numChunks, cgData.numPoints, cgData.gpuName.c_str()));
                break;
            }
        ...
        logAdd(LogLevel::Info, "GPU Convolution is done.");
    }
    ...
    logAdd(LogLevel::Info, "Writing to the output file...");
    {
        // status
