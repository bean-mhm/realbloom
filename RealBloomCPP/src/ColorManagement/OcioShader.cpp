...
OcioShader::~OcioShader()
{
    ...
    for (size_t idx = 0; idx < max; ++idx)
    {
        ...
        checkGlStatus(__FUNCTION__, formatStr("glDeleteTextures (idx: %u)", idx));
    }
    m_textureIds.clear();
...
void OcioShader::prepareLuts()
{
    ...
    for (uint32_t idx = 0; idx < maxTexture3D; ++idx)
    {
            ...
            || edgelen == 0)
        {
            throw OCIO::Exception(
                printErr(__FUNCTION__, formatStr("The texture data is corrupted (3D LUTs, idx: %u)", idx)).c_str()
            );
        }
        ...
        {
            throw OCIO::Exception(
                printErr(__FUNCTION__, formatStr("The texture values are missing (3D LUTs, idx: %u)", idx)).c_str()
            );
        }
    ...
    for (uint32_t idx = 0; idx < maxTexture2D; ++idx)
    {
        ...
        {
            throw OCIO::Exception(
                printErr(__FUNCTION__, formatStr("The texture data is corrupted (1D LUTs, idx: %u)", idx)).c_str()
            );
        }
        ...
        {
            throw OCIO::Exception(
                printErr(__FUNCTION__, formatStr("The texture values are missing (1D LUTs, idx: %u)", idx)).c_str()
            );
        }
...
void OcioShader::useLuts()
{
    ...
    for (size_t idx = 0; idx < max; ++idx)
    {
        ...
        checkGlStatus(__FUNCTION__, formatStr("glActiveTexture(%u) (idx: %u)", texUnit, idx));

        glBindTexture(data.m_type, data.m_uid);
        checkGlStatus(__FUNCTION__, formatStr("glBindTexture(%u, %u) (idx: %u)", data.m_type, data.m_uid, idx));

        GLint loc = glGetUniformLocation(m_program, data.m_samplerName.c_str());
        checkGlStatus(__FUNCTION__, formatStr("glGetUniformLocation(%s) (idx: %u)", data.m_samplerName.c_str(), idx));

        glUniform1i(loc, GLint(m_lutStartIndex + idx));
        checkGlStatus(__FUNCTION__, formatStr("glUniform1i(%d, %d) (idx: %u)", loc, GLint(m_lutStartIndex + idx), idx));
    }
}
...
void OcioShader::prepareUniforms()
{
    ...
    for (uint32_t idx = 0; idx < maxUniforms; ++idx)
    {
        ...
        {
            throw OCIO::Exception(
                printErr(__FUNCTION__, formatStr("Unknown uniform type (idx: %u)", idx)).c_str()
            );
        }
        // Transfer uniform.
    ...
    uint32_t edgelen, const float* values)
{
    if (values == 0x0)
        throw OCIO::Exception(
            printErr(__FUNCTION__, formatStr("Missing texture data (idx: %u)", index)).c_str()
        );

    glGenTextures(1, &texId);
    checkGlStatus(__FUNCTION__, formatStr("glGenTextures (idx: %u)", index));

    glActiveTexture(GL_TEXTURE0 + index);
    checkGlStatus(__FUNCTION__, formatStr("glActiveTexture (idx: %u)", index));

    glBindTexture(GL_TEXTURE_3D, texId);
    checkGlStatus(__FUNCTION__, formatStr("glBindTexture (idx: %u)", index));

    setTextureParameters(GL_TEXTURE_3D, interpolation);
    checkGlStatus(__FUNCTION__, formatStr("setTextureParameters (idx: %u)", index));

    glTexImage3D(GL_TEXTURE_3D, 0, GL_RGB32F, edgelen, edgelen, edgelen, 0, GL_RGB, GL_FLOAT, values);
    checkGlStatus(__FUNCTION__, formatStr("glTexImage3D (idx: %u)", index));
}
    ...
    OCIO::Interpolation interpolation, const float* values)
{
    ...
    if (height > 1)
    {
        ...
        checkGlStatus(__FUNCTION__, formatStr("glTexImage2D (idx: %u)", index));
    } else
    {
        ...
        checkGlStatus(__FUNCTION__, formatStr("glTexImage1D (idx: %u)", index));
    }
}
...
void OcioShader::Uniform::setUp(uint32_t program)
{
    ...
    checkGlStatus(__FUNCTION__, formatStr("glGetUniformLocation(%s)", m_name.c_str()));
}
...
void OcioShader::Uniform::use()
{
    ...
    } else if (m_data.m_vectorInt.m_getSize && m_data.m_vectorInt.m_getVector)
    {
        ...
        throw OCIO::Exception(
            printErr(__FUNCTION__, formatStr("Uniform %s is not linked to any value.", m_name.c_str())).c_str()
        );
    checkGlStatus(__FUNCTION__, formatStr("\"%s\"", m_name.c_str()));
}
